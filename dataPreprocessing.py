...
def init():
    ...
    def preprocessFile(tf,topic):
        ...
        for entry in tf.itertuples():
            ...
            cwordsintags= tokenizer.tokenize(wordsintags)
            ...
            if (incorporate):
                ...
                ccontent=nltk.word_tokenize(htmlcontent.get_text().lower().encode('utf-8').decode('utf-8'))
                #TITLE
                title = entry[settings.CONTENT_TITLE].lower().encode('utf-8').decode('utf-8')+"."
                #ctitle = cleanText(nltk.word_tokenize(title))
                ctitle=nltk.word_tokenize(title)
